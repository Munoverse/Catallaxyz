...
/**
 * Initialize Creator Treasury
 *
 * This script will:
 * 1. Initialize the creator treasury token account
 * 2. This account collects the creator incentive share of trading fees
 ...
 *
 * Usage:
 *   yarn ts-node scripts/initialize-creator-treasury.ts
 */
...
async function main() {
  ...
  console.log("👤 Creator Treasury PDA:", creatorTreasuryPda.toString());
  console.log("");

  try {
    const accountInfo = await connection.getAccountInfo(creatorTreasuryPda);
    if (accountInfo && accountInfo.data.length > 0) {
      console.log("⚠️  Creator Treasury already initialized!");
      console.log("   Account exists with", accountInfo.data.length, "bytes");

      try {
        const tokenAccount = await connection.getTokenAccountBalance(creatorTreasuryPda);
  ...
  try {
    ...
    if (accountInfo) {
      console.log("✅ Creator Treasury account created");
      console.log("   Address:", creatorTreasuryPda.toString());
      console.log("   Owner:", accountInfo.owner.toString());
      console.log("   Size:", accountInfo.data.length, "bytes");

      try {
        const tokenBalance = await connection.getTokenAccountBalance(creatorTreasuryPda);
    ...
    console.log("✨ Creator Treasury initialization complete!");
    console.log("=".repeat(60));
    console.log("");
  } catch (error: any) {
