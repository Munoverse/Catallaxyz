 ...
 * 
 * Usage:
 *   yarn ts-node scripts/initialize-treasury.ts
 */
...
async function main() {
  ...
  console.log("🏦 VRF Treasury PDA:", treasuryPda.toString());
  console.log("");
  ...
  try {
    ...
    if (accountInfo && accountInfo.data.length > 0) {
      console.log("⚠️  VRF Treasury already initialized!");
      console.log("   Account exists with", accountInfo.data.length, "bytes");
      
      // Try to parse as token account
      try {
        const tokenAccount = await connection.getTokenAccountBalance(treasuryPda);
  ...
  try {
    ...
    if (accountInfo) {
      console.log("✅ VRF Treasury account created");
      console.log("   Address:", treasuryPda.toString());
      console.log("   Owner:", accountInfo.owner.toString());
      console.log("   Size:", accountInfo.data.length, "bytes");
      
      try {
        const tokenBalance = await connection.getTokenAccountBalance(treasuryPda);
    ...
    console.log("✨ VRF Treasury initialization complete!");
    console.log("=".repeat(60));
    console.log("");
    console.log("📝 Next Steps:");
    console.log("1. Verify all accounts:");
